...
from settings import llm, ExecutorAgent_prompt
from services.agents.agent_state import AgentState

_LOGGER = logging.getLogger(__name__)


class ExecutoragentAgent:
    def __init__(self):
        pass

    async def create(self):
        tools = []
        for server_name in ['sberdru_server', 'sberusersoft_server', 'pulse_server']:
            tools += await client.get_tools(server_name=server_name)
        self.agent = create_react_agent(llm, tools, checkpointer=MemorySaver(), prompt=ExecutorAgent_prompt)
        self.config = {
                "configurable": {
                "thread_id": str(uuid4())
            },
