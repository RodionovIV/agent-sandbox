...
from settings import llm, AgentCoordinator_prompt
...
class AgentcoordinatorAgent:
    ...
    async def create(self):
        tools = []
        for server_name in ['api_server']:
            tools += await client.get_tools(server_name=server_name)
        self.agent = create_react_agent(llm, tools, checkpointer=MemorySaver(), prompt=AgentCoordinator_prompt)
        self.config = {
                "configurable": {
                "thread_id": str(uuid4())
            },
