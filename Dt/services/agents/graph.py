...
from services.agents.explainer_agent import ExplainerAgent
...
def orchestrator_node(agent):
    async def create_node(state:AgentState)->Command[Literal["analyst", "explainer", END]]:
...
def explainer_node(agent):
    async def create_node(state:AgentState)->Command[Literal["orchestrator"]]:
        node_name = "explainer_node"
        task_field = "explainer_task"
        result_field = "explainer_result"
        state["previous"] = "explainer"
        print(f"Status: {node_name}")
...
async def create_graph():
    ...
    builder.add_node("explainer", explainer_node(_explainer_agent))
