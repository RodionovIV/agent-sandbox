...
from settings import llm, analyzer_prompt
...
class AnalyzerAgent:
    ...
    async def create(self):
        tools = []
        for server_name in ['server1']:
            tools += await client.get_tools(server_name=server_name)
        self.agent = create_react_agent(llm, tools, checkpointer=MemorySaver(), prompt=analyzer_prompt)
        self.config = {
                "configurable": {
                "thread_id": str(uuid4())
            },
