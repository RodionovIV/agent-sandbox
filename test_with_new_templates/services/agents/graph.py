...
def orchestrator_node(agent):
    async def create_node(state:AgentState)->Command[Literal["analyzer", END]]:
...
def analyzer_node(agent):
    async def create_node(state:AgentState)->Command[Literal["orchestrator"]]:
        node_name = "analyzer_node"
        task_field = "analyzer_task"
        result_field = "analyzer_result"
        state["previous"] = "analyzer"
        print(f"Status: {node_name}")
...
async def create_graph():
    ...
    await _analyzer_agent.create()
    builder = StateGraph(state_schema=AgentState)
    builder.set_entry_point("orchestrator")
    builder.add_node("orchestrator", orchestrator_node(_orchestrator_agent))
    builder.add_node("analyzer", analyzer_node(_analyzer_agent))
